...
| 1.4a | Unforgeability is a **design goal** under the Module-LWE / Module-SIS assumptions, not an achieved fact | FIPS 204 §3.4, §4 | `provenance.spec.ts` — the hero must say "designed" and name the assumption; no page may state an unhedged absolute | The assumptions are unproven. No signature scheme can claim more |
...
| 9.6 | Lighthouse: performance ≥ 0.90, **accessibility 1.00**, best-practices ≥ 0.90, SEO ≥ 0.90 over three runs | — | `scripts/lighthouse.mjs` in CI, median for timing categories and **worst run** for accessibility | Performance had sat exactly at its budget on CI hardware; splitting the four non-landing panels cut initial JS from 145 KB to 85 KB (45.5 → 28.1 KB gzipped) for headroom. SEO is capped at 0.91 by a `robots.txt` this repository cannot serve |
| 9.6a | Panels load on demand without breaking | — | `resilience.spec.ts` — the landing tab makes exactly one JS request; each other tab fetches its chunk once and not again on revisit; a failed chunk says so instead of showing an empty tab; two fast clicks land on the last one | A slow connection now shows a brief empty panel on first visit to a tab |
...
7. ~~A thin performance margin.~~ **Improved** — initial JavaScript cut by 42% by loading the four non-landing panels on demand. The budget is still a budget, and a sufficiently slow runner can still miss it.
