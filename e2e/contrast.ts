 ...
 *    `color-mix()`. This page's surfaces are mostly opaque hexes, but three of
 *    the places it renders a VERDICT are not: `.badge-pass` and `.badge-fail`
 *    are `rgba(…, 0.1)` tints whose own comment records that 0.15 pushed their
 *    text under 4.5:1, and `.cl-hero-why` is a
 *    `color-mix(in oklab, var(--accent) 6%, transparent)`. axe files those under
 *    `incomplete` rather than measuring them, and they are exactly the strings
 *    the lab exists to show: "✓ VERIFIED", "✗ FAILED", "✗ REJECT — ‖z‖∞ = …".
 ...
 *    `.btn:disabled { opacity: 0.5 }`, which this page leans on heavily: seven
 *    of the nine controls in the Sign & Verify tab ship disabled and re-lock
 *    whenever the parameter set changes. Those are genuinely inactive
 *    components, so the exemption below clears them honestly — but it clears
 *    them by reading `disabled`, not by overriding the opacity, which is the
 *    difference between measuring the page and rewriting it.
 *
 ...
 *    boxes are `.output` (the hex dumps, capped at 160px of height), the
 *    `.math-block` algebra lines, `#tabs`, and `.comparison-table`, which turns
 *    into its own `overflow-x` scroller below 768px. Content scrolled past them
 *    is
 ...
 *    marks, both `aria-hidden` and glyph-free, and every one of its own figures
 *    (the size bar charts, the coefficient plots, the timeline) is built from
 *    styled `<div>`s. So the sibling walk finds nothing to do. It is kept
 ...
 * generated content. It iterates ELEMENTS, and a `::before` is not one — neither
 * is it one to axe's `color-contrast` rule. This page numbers its five-step
 * explainer and its seven-step workflow entirely from
 * `content: counter(step)` / `content: counter(wf)`, so those digits are
 * unmeasured by both oracles. All four states they can take were measured by
 * hand instead, from real screenshot pixels: `.step::before` idle is 6.45:1
 * (dark) / 5.67:1 (light), `.step.active::before` 6.03:1 / 6.17:1, and
 * `.wf-step::before` 6.03:1 / 6.17:1 — all four clear the 4.5:1 their 12-12.8px
 * digits require.
 */
 ...
 * uses. The only styling here that lives briefly is the `.spinner` line inside
 * a `.output`/`aria-live` region while a keypair, a signature or a benchmark is
 * computed, and its text is plain `--text` on the same card the settled result
 * lands on — so a whole-page scan of the settled state covers the same pair.
 * The parameter is
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven scan of the Fiat-Shamir animator walks three 12-coefficient bar
     * plots per signing attempt, each column its own `role="img"` group with a
     * printed value, for as many attempts as the abort loop needed; the Compare
     * tab adds seven-row bar charts twice over and two tables; and every one of
     * those nodes re-walks the same handful of ancestors up to `<body>`. Without
     * the caches the pass re-reads the same computed styles and rects thousands
     * of times, and the gate scans about thirty states per configuration.
     ...
     * that is `.output`, `.math-block`, `#tabs` and `.comparison-table`.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `<details class="math-details">`, both shut on arrival and both inside
      // a collapsed stepper step; the gate opens each by clicking its
      // `<summary>` after opening the step that holds it — the route a reader
      // has — rather than setting `.open` from script and adding `.active` to
      // every step at once, which is what the previous gate did.
      ...
      // page has two: the shared header's `.cl-skip-link` parks at `top:-3rem`
      // and the lab's own `.skip-link` at `left:-9999px`, each sliding in only
      // on focus. Measuring a parked copy invents a failure for text that is not
     ...
     * `aria-hidden` but still painted is skipped by BOTH oracles. This lab uses
     * the attribute in three places and each was checked rather than assumed:
     * the shared header's glyph-free `<svg>`s; the `.intuition-icon` emoji,
     * which carry no information the sentence beside them does not; and the
     * three spans of a `.bar-row`, whose scheme name, bar and byte count are
     * hidden only because the row itself is a `role="img"` whose `aria-label`
     * repeats all three verbatim. That last one is the risky shape, so its ink
     * was measured by hand: `.bar-label` and `.bar-value` are `--text-muted`
     * and `--text` on `--surface`, the same pairs the measured prose beside
     * them uses.
     */
     ...
     * cheap to make and silent, and this page builds every one of its tabs by
     ...
     * group. They are invisible on screen, but a walk that measures them
     * reports a 1:1 failure against the panel — a ratio describing ink that was
     * never laid down.
     */
